...
export default async function NewsContent() {
  ...
  const newsItems = blogPosts.map((post) => ({
    ...
    slug: post.slug,
    image:
      post.imageUrl || getFirstImageFromContent(post.content) || "/news.jpg",
  ...
  const fallbackNews = [
    {
      ...
      date: "April 15, 2023",
      content:
        "The Lumbini Lions have completed the signing of star batsman Rajesh Kumar from rivals Chitwan Rhinos for an undisclosed fee. Kumar, who scored 412 runs in last season's tournament, has signed a two-year contract with the Lions. This acquisition is expected to significantly strengthen the Lions' batting lineup for the upcoming season.",
    },
      ...
      <div className="flex justify-center h-full w-full min-h-screen z-[10] pt-40 md:pt-10 pt-[-10px] overflow-hidden relative">
        ...
        <div className="absolute top-0 left-0 right-0 w-screen -z-10 ">
          <div className="relative w-full">
            <h2 className="text-2xl absolute top-15 left-15 md:left-40 md:text-4xl font-bold font-['poppins'] z-10 text-white">
              LATEST NEWS
            </h2>
            {/* <Link href="/news">
